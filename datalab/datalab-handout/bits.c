...
分为几种情况
1. x=0,y=0
2. x=0,y=1 or x=1,y=0
3. x=1,y=1
情况1：x，y均为0时，~x&~y=1，~(x&y)=1，~(~x&~y)&~(x&y)=0
情况2：x，y有且仅有一个为0时，~x&~y=1，~(x&y)=1，~(~x&~y)&~(x&y)=1
情况3：x，y均为1时，~x&~y=0，~(x&y)=0，~(~x&~y)&~(x&y)=0
*/
}
/* 
...
int floatFloat2Int(unsigned uf) {
  ...
  int exp = ((uf >> 23) & 0xFF) - 127; // 提取指数和偏置
  unsigned frac = (uf & 0x7FFFFF) | 0x800000; // 提取尾数并设置隐含的首位1
